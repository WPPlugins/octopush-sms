    ...
    abstract class Octopush_Sms_Generic_Campaign_Tab {
        ...
        protected function get_body_campaigns() {
            ...
            if (get_class($this) == 'Octopush_Sms_Send_Tab') {
                $html .='<h2>' . __('Campaigns that have not yet been sent', 'octopush-sms');
            } else { 
            ...
            $myListTable->search_box(__('Search Campaigns', 'octopush-sms'), 'search_id');
            echo '</form>';
            $myListTable->display();
            $html .= ob_get_clean();
            $html.='</div>';
        ...
        public function output() {
            ...
            <div class="postbox-container">
                <div id="sendsms_message" class="postbox doc">
                    ...
                    <?php if (get_class($this) == 'Octopush_Sms_Send_Tab') { ?>
                        <p><?php
                            echo __('As soon as you start choosing the recipient(s), your campaign will be automatically saved and you will find it in "Send SMS" tab for further modification if necessary.', 'octopush-sms') . '<br/>' .
                            __('Here is the sending process :', 'octopush-sms') . '<br/>' .
                            __('1. Create your campaign, click on \'Save\' if you want to be able to modify it later.', 'octopush-sms') . '<br/>' .
                            __('2. When your campaign is ready, click "Transmit to Octopush" to send your request to the SMS plateform, SMS will not be send at this time ! Status become "Transfert in progess" and the campaign can\'t be modified anymore.', 'octopush-sms') . '<br/>' .
                            ...
                            __('4. If there\'s no error (insuffisant balance for example), your campaign will be sent. Else, please fix the problem and try again.', 'octopush-sms') . '<br/>' .
                            __('5. You can retrieve all the campaigns whose are not in the status "In Construction", "Transfer in progress" or "Waiting for validation" in the "SMS history" tab.', 'octopush-sms') . '<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' .
                            __('- You can still cancel it unless it has been sent by Octopush.', 'octopush-sms');
                            ?>
                        </p>
                    <?php } else { ?>
                        <br><br>
                                        ...
                                        <div class="sendsms_data_column_container">
                                            <div class="sendsms_data_column">
                                                ...
                                                <p class="form-field form-field-wide"><label for="sendsms_date"><?php _e('Send date', 'octopush-sms') ?></label><br/>
                                                    <input type="text" <?php echo ($this->_campaign->status < 2 ? '' : 'readonly') ?> class="date-picker-field datepicker" name="sendsms_date" id="sendsms_date" maxlength="10" value="<?php echo date_i18n('Y-m-d', strtotime($this->_campaign->date_send)); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
                                                    @<input type="text" <?php echo ($this->_campaign->status < 2 ? '' : 'readonly') ?> class="hour" style="width:2.5em" placeholder="<?php _e('h', 'octopush-sms') ?>" name="sendsms_date_hour" id="sendsms_date_hour" maxlength="2" size="2" value="<?php echo date_i18n('H', strtotime($this->_campaign->date_send)); ?>" pattern="\-?\d+(\.\d{0,})?" />
                                                    :<input type="text" <?php echo ($this->_campaign->status < 2 ? '' : 'readonly') ?> class="minute" style="width:2.5em" placeholder="<?php _e('m', 'octopush-sms') ?>" name="sendsms_date_minute" id="sendsms_date_minute" maxlength="2" size="2" value="<?php echo date_i18n('i', strtotime($this->_campaign->date_send)); ?>" pattern="\-?\d+(\.\d{0,})?" />
            ...
            if ($this->_campaign->status == 0) {
                $jsScript .= 'var timeText = "' . __('Time', 'octopush-sms') . '";
                    ...
                    var closeText = "' . __('Close', 'octopush-sms') . '";';
            }
            $jsScript .= '
        ...
        <label><b>' . __('Amount in your account', 'octopush-sms') . '</b></label>
        ...
        <div class="clear"></div>
        <label>' . __('Send date', 'octopush-sms') . '</label>
        <div>' . ($this->_campaign->date_send != "0000-00-00 00:00:00" ? date_i18n($this->_campaign->date_send) : '') . '</div>
        <div class="clear"></div>
        <label>' . __('Transmition date', 'octopush-sms') . '</label>
        <div>' . ($this->_campaign->date_transmitted != "0000-00-00 00:00:00" ? date_i18n($this->_campaign->date_transmitted) : '') . '</div>
        <div class="clear"></div>
        <label>' . __('Validation date', 'octopush-sms') . '</label>
        <div>' . ($this->_campaign->date_validation != "0000-00-00 00:00:00" ? date_i18n($this->_campaign->date_validation) : '') . '</div>
    </fieldset>';
        }
            ...
            if (get_class($this) == 'Octopush_Sms_Send_Tab' && $this->_campaign->status == 0) {
                ...
                <div class="poststuff">
                    <div id="sendsms_choose_recipient" class="postbox">
                        <h2><span class="dashicons dashicons-search vmiddle"></span><?php _e('Choose the recipients', 'octopush-sms') ?></h2>
                            ...
                            <tbody>
                                <tr valign="top">
                                    ...
                                    <td class="forminp" style="width:80%">
                                        <input placeholder="<?php _e('Phone (international e.g: +33612345678)', 'octopush-sms') ?>" type="text" size="30" maxlength="16" name="sendsms_phone" id="sendsms_phone" />
                                        <input placeholder="<?php _e('Firstname (optional)', 'octopush-sms') ?>" type="text" size="30" maxlength="32" name="sendsms_firstname" id="sendsms_firstname" /></div>
                                        <input placeholder="<?php _e('Lastname (optional)', 'octopush-sms') ?>" type="text" size="30" maxlength="32" name="sendsms_lastname" id="sendsms_lastname" /></div>
                                        <span class="plus" id="add_recipient"></span>
                                    </td>
                                </tr>
                                <tr valign="top">
                                    <th scope="row" class="titledesc">
                                        <label for="sendsms_customer"><span class="dashicons dashicons-arrow-right"></span><?php _e('Or search for customers', 'octopush-sms') ?></label>
                                    </th>
                                ...
                                <tr valign="top" class="">
                                    ...
                                    <td class="forminp forminp-checkbox" >
                                        ...
                                        <br/>
                                        <?php _e('Phone in 1st column (international format), Firstname in 2nd (optional), Lastname in 3rd (optional)', 'octopush-sms') ?> 
                        ...
                        <?php if ($this->_campaign->status == 0) { ?>
                            <div style="line-height:30px"><span class="dashicons dashicons-info"></span> <?php _e('All doublons will be automatically removed', 'octopush-sms') ?></div>
                        <?php } ?>
