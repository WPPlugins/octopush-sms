...
class OWS
{
	...
	private function _httpRequest($domain, $path, $port, $A_fields = array())
	{
		...
		$request = join('&', $qs);
                if (WP_DEBUG) error_log ("_httprequest: $ssl $port $request");
		...
		if (function_exists('curl_init') AND $ch = curl_init(substr($domain, _CUT_) . $path))
		{
			...
			curl_close($ch);
                        ...
                        if (WP_DEBUG) error_log("_httprequest result: $result");
		...
		else if (ini_get('allow_url_fopen'))
		{
			...
			if (!$fp)
			{
				if (WP_DEBUG) 'Unable to connect to host. Try again later.';
		...
		{
                    if (WP_DEBUG) error_log('Server does not support HTTP(S) requests.');     
                    die('Server does not support HTTP(S) requests.');
		}
                if (WP_DEBUG) error_log("_httprequest response: $response");
