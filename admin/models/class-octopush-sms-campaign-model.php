...
if (!class_exists('Octopush_Sms_Campaign_Model')) {
    ...
    class Octopush_Sms_Campaign_Model {
        ...
        public function get_recipients($page_nbr = 1, $per_page = 1000, $orderby = null, $order = null, $s = null) {
            ...
            /* if (intval($this->id_sendsms_campaign)) {
              $sql = "select * from " . Octopush_Sms_Recipient::_get_table() . " where id_sendsms_campaign = " . (int) $this->id_sendsms_campaign;
              if (WP_DEBUG)
              error_log($sql);
        ...
        public function Octopush_Sms_Campaign_Model($id_sendsms_campaign = null) {
            //default value
            $this->status = 0;
            $this->error_code = '';
