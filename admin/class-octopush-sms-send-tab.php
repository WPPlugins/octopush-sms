            ...
            if (isset($this->post['sendsms_save']) && $this->_campaign->status == 0) {
                ...
                if ($this->_campaign->nb_recipients > 0) {
                    ...
                    if ($xml->error_code == '000' || intval($xml->error_code) == _ERROR_BATCH_SMS_NOT_FOUND_) {
                        ...
                        $this->_campaign->save();
                        self::add_message(__('Your campaign is now canceled on octopushsms and can be deleted', 'octopush-sms', 'octopush-sms'));
                    } else {
                    ...
                    $this->_campaign->status = 4;
                    $this->_campaign->save();
                    self::add_message(__('Your campaign is now canceled and can be deleted', 'octopush-sms'));
                }
                ...
                self::add_message(__('Your campaign has been duplicated, you are now working on the new one.', 'octopush-sms'));
        ...
        public static function _ajax_process_addRecipientsFromQuery() {
            ...
            if (is_array($result)) {
                ...
                foreach ($result as $row) {
                    ...
                    $recipient->lastname = $row->billing_lastname != '' ? $row->billing_firstname : $row->display_name;
