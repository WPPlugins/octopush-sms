	...
	class Octopush_Sms_Messages extends WC_Settings_Page
	{
		...
		public function getBody()
		{
			...
			<div id="' . get_class($this) . '">
				<br /><b>' . __('Choose SMS you want to activate, and customize their text.', 'octopush-sms') . '</b><br /><br />';
			$html .=
				__('On the right side you can see a preview to check that everything is ok.', 'octopush-sms') . '<br /><br />
			<div class="clear"></div>
				<div class="wrap woocommerce">
					<form id="' . get_class($this) . '_form" method="post">
						<input type="hidden" name="action" value="' . get_class($this) . '"/>
						<h3>' . __('SMS for Admin', 'octopush-sms') . '</h3>
						<div class="os_row">' .
							$admin_html . '
						</div>
						<br />
						...
						<br /><br />
						<h3>' . __('SMS for Customer', 'octopush-sms') . '</h3>
						<div class="os_row">' .
							$customer_html . '
						</div>
						<br />
		...
		<div class="ows">
			...
			{
				$code .= '<br/><span style="font-weight: normal">' . __('Sent only if customer pay the option', 'octopush-sms') . '</span>';
			}
			$code.='</label>
				</th>
				<td class="forminp forminp-' . $hookId . ' data_td">';
			$code .= '<input ' . (get_option($keyActive) == 1 ? 'checked' : '') . ' type="checkbox" name="' . $keyActive . '" value="1"/> ' . __('Activate', 'octopush-sms') . ' ?<br/>';
			...
			$code .= '<textarea name="' . $key . '" rows="4" class="message_textarea">' . $txt
				...
				(!$bGSM7 ? '<img src="../img/admin/warning.gif"> ' . __('This message will be divided in 70 chars parts, because of non standard characters : ', 'octopush-sms') . ' ' . Octopush_Sms_API::get_instance()->not_GSM7($txt_test) : __('This message will be divided in 160 chars parts', 'octopush-sms')) .
				'</span>'
				. '<br/>';
